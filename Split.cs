...
namespace UnitySimpleLiquid
{
	...
	public class Split : MonoBehaviour
	{
		...
		public ParticleSystem coppersulphate;
		private ParticleSystem copperparticleInstance;

		public ParticleSystem ironpowder;
		...
		{
			get { return Particles.main.startColor.color; }
			//set { particles.main.startColor = value; }
		}
		...
		private void OnTriggerEnter(Collider other)
		{
			...
			if (asset != null)
			{
				...
				{
					particles.Stop(); // Stop the current particle system
					Destroy(particles.gameObject);

					particles = Instantiate(coppersulphate, transform);
				}
				else if (assetName == "ironbottle")
				{
					...
					particles = Instantiate(ironpowder, transform);
				}
			}
		}
		...
		void StartEffect(Vector3 splitPos, float scale)
		{
			...
			mainModule.startColor = fluidContainer.LiquidColor;
		...
		private Plane GenerateBottleneckPlane()
		{
			...
			var mesh = fluidContainer.LiquidMesh;
			if (!mesh)
				return new Plane();

			var max = mesh.bounds.max.y;
			return new Plane(fluidContainer.transform.up,
				max * fluidContainer.transform.lossyScale.y);
		}
		...
		private Vector3 GenerateBottleneckPos()
		{
			...
			var tr = fluidContainer.transform;
		...
		private void CheckSpliting()
		{
			...
			if (fluidContainer.FillAmountPercent <= 0f)
				return;
			if (!fluidContainer.IsOpen)
				return;
			...
			overflowsPoint += fluidContainer.transform.position;
			...
			if (BottleneckPos.y < overflowsPoint.y)
			{
				...
				else
				{
					...
					var dist = fluidContainer.liquidRender.bounds.SqrDistance(overflowsPoint);
		...
		private void SplitLogic(Vector3 splitPos)
		{
			...
			var howLow = Vector3.Dot(Vector3.up, fluidContainer.transform.up);
			var flowScale = 1f - (howLow + 1) * 0.5f + 0.1f;

			var liquidStep = BottleneckRadiusWorld * splitSpeed * Time.deltaTime * flowScale;
			var newLiquidAmmount = fluidContainer.FillAmountPercent - liquidStep;

			// Check if amount is negative and change it to zero
			if (newLiquidAmmount < 0f)
			{
				liquidStep = fluidContainer.FillAmountPercent;
				newLiquidAmmount = 0f;
			}

			// Transfer liquid to other container (if possible)
			fluidContainer.FillAmountPercent = newLiquidAmmount;

			RaycastHit containerHit = FindFluidContainer(splitPos, this.gameObject);
		...
		{
			var liquid = hit.collider.GetComponent<Split>();
			...
			if (insideRadius)
			{
				var lostAmount = fluidContainer.Volume * lostPercentAmount;
				liquid.fluidContainer.FillAmount += lostAmount;

				//color change in capacity
				SendFluidContainer(liquid.fluidContainer);
			}


		}

		private RaycastHit FindFluidContainer(Vector3 splitPos, GameObject ignoreCollision)
		{
				...
				if (!GameObject.ReferenceEquals(hit.collider.gameObject, ignoreCollision) && !hit.collider.isTrigger)
				{
					...
					if (!liquid)
					{
						...
						if (edgePosition != Vector3.zero)
						{
							...
							currentDrop++;
							return FindFluidContainer(edgePosition, hit.collider.gameObject);
		...
		private void SendFluidContainer(FluidContainer lc)
		{
			//find the color and split speed
			Color newColor = fluidContainer.LiquidColor;
			float ss = fluidContainer.GetSplit.splitSpeed;
		...
		private Vector3 TryGetSlopeEdge(Vector3 slope, RaycastHit hit)
		{
			...
			Vector3 moveDown = new Vector3(0f, -0.1f, 0f);
		...
		public void Update()
		{
			...
			surfacePlane = fluidContainer.GenerateSurfacePlane();
			BottleneckRadiusWorld = bottleneckRadius * transform.lossyScale.magnitude;
